...
test('intersectSegmentsMatcher benchmark', () => {
  const n = 1000;
  let r1: any;

  const t1 = Date.now();
  for (let i = 0; i < n; i += 1) {
    r1 = intersectSegmentsMatcher(
  ...
  const t3 = Date.now();
  for (let i = 0; i < n; i += 1) {
    r1 = intersectSegmentsMatcher(matcher1, matcher2);
  }
  const t4 = Date.now();
